...
1. В ОС есть специальная директория /dev. Это директория в которой лежат файлы устройств. 
...
content for sdtin
```
То есть если мы из процесса с PID 1543 выполним cat /dev/stdin, то по файкту мы обратимся к дискриптору /proc/1543/fd/0 за которым может быть например pipe.
И считать переданную в него (в pipe) строку. 
4. Это означает, что у ядра ОС есть реальные структуры типа:
- файл
- терминал
- pipe
- сокет

И при запуске нового процесса, определяется какая то из этих структур (как источник или приемник) за дескриптором. 
То есть процесс просто пишет или читает в файловый дескриптор, но что за ним стоит он не знает. 
Например если перенаправить вывод ошибок (sdterr) и просто вывод (stdout) в файлы, то он не попадет в терминал.
Создадим два файла
```bash
touch logsuccess
...
Вывод: При создании процесса за дескрипторами устанавливается страктура ядра. По дефолту это терминал (если вызов в терминале). 
Процесс просто пишет в дескриптор ,а за ним кроется та или иная структура ядра. Файлы /dev/stdin, /dev/stdout и /dev/stderr это просто ссылки на дескрипторы в виде файла, чтобы удобно можно было читать или писать из программы.
