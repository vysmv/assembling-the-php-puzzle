...
И теперь, если мы перейдём по адресу `http://localhost:8888/`, то увидим приветственную страницу веб-сервера Apache2.
...
- `cat -` — читает данные из stdin (читает и многострочный ввод; источником может быть как pipe, так и файлы)
    ...
    Мы можем либо передать содержимое stdin сразу через pipe или файл (`./script.bash < file_name`),  
...
1. Как уже говорилось ранее, каждый процесс наследует таблицу файловых дескрипторов от родительского процесса (обычно shell/bash), который, в свою очередь, формирует их при запуске. Это:
- 0 (stdin)
- 1 (stdout)
- 2 (stderr)
...
Например, если у нас есть скрипт на bash:
```bash
#!/usr/bin/env bash
cat < /dev/stdin
```
...
В PHP-скриптах могут быть проблемы при попытке чтения или записи в файлы /dev/stdin и /dev/stdout, но там есть удобные псевдопотоки типа `php://stdin` и `php://stdout`. На практике нужно использовать тот вариант работы с I/O, который предоставляет интерпретатор. 

### CLI профиль

Но давайте снова вернёмся к нашему скрипту на `PHP`.
...
В протоколе CGI просто парсится HTTP-запрос, и стартовая строка с заголовками идёт в переменные окружения, а тело запроса — в stdin.
