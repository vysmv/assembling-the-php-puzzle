...
### Общий принцип работы c вводом и выводом в Unix-подобных системах

1. Каждый процесс запускается со своей таблицей файловых дескрипторов. Это:
- 0 (sdtin)
- 1 (sdtout)
- 2 (stderr) 
Процесс просто пишет что-то в дескрипторы 1 и 2 или читает из дескриптора 0. Но он не знает, куда реально идут и от куда приходят данные. По умолчанию это вывод в терминал или ввод с клавиатуры. 
Что именно будет выступать в качестве источника для дескриптора 0 и приемником для дескрипторов 1 и 2, определяется на старте процесса.

У ядра ОС есть набор структур типа:
- файл
- терминал
- pipe
- сокет
...
И любая из них может использоваться как источник\приемник.
...
content for sdtin
```
То есть мы отправили в pipe строку "content for sdtin" и этот пайп был установлен при запуске процесса bash исполняющего script.bash как источник для дескриптора 0.
Потом мы читаем псевдофайл /dev/stdin который является ссылкой на проекцию дескриптора `/proc/self/fd/0`, где self заменяется на реальный PID текущего процесса.
И в итоге через проекцию дескриптора получаем содержание пайпа который является источником дескриптора 0 у текущегго процесса. 
...
Или как вариант мы можем поступить так, перенаправить вывод ошибок (sdterr) и просто вывод (stdout) в файлы. 
Создадим два файла
```bash
touch logsuccess
...
```
То в зависимости от аргумента (1 или 0) будет происходить запись в разне файлы. 
А в терминале не будет ничего печаться. 
То есть мы заменили при старте процесса приемники которые кроются за дескрипторами 1 и 2 на файлы logsuccess и logerror. 

Вывод: При создании процесса за дескрипторами устанавливается та или иная страктура ядра. По дефолту это терминал (если вызов в терминале). 
Процесс просто пишет в дескриптор (write(1, ...)), а за ним кроется та или иная структура ядра. Файлы /dev/stdin, /dev/stdout и /dev/stderr это просто ссылки на проекцию дескриптор текущего процесса.
