...
После того как мы разобрали "Запуск программ в терминале", "Общий принцип работы с вводом и выводом в Unix-подобных системах", "CLI профиль" пришло время снова вернуться к Apache2 и CGI.  
...
В протоколе CGI просто парсится HTTP-запрос, и стартовая строка с заголовками устанавливается в переменные окружения, а тело запроса — в stdin.
...
То есть сделаем запрос к веб-серверу через http клиент, а веб-сервер вызовет файл php-cgi-runner как самостоятельный процесс. 
В файле php-cgi-runner указан шеба́нг #!/usr/bin/env php-cgi 
То есть реально будет запушен интерпретатор php-cgi.
Он считает переменные окружения и поймет на основе содержания заголовка `SCRIPT_FILENAME` какой файл мы пытаемся запустить. 
И самостоятельно исполнит его. 
Выполним запрос `http://localhost:8888/script.php?name=ivan&age=32`.


Резюме:  
Мы видим, что условный "веб-профиль", созданный CGI SAPI, также соответствует нашим ожиданиям.  
В нём заполнены все суперглобальные массивы.  
А работа с вводом и выводом идёт по псевдопотокам `php://input` и `php://output`.  
Константы типа `STDIN` пустые.  
Псевдопотоки типа `php://stdin` обнулены.  
В CGI работа с вводом и выводом унифицирована с FastCGI.  
То есть специально отключены константы и псевдопотоки типа `php://stdin` для единообразия работы в вебе.
...
Этот запущенный процесс выполнит свой алгоритм и вернёт ответ.  
Этот ответ может содержать желаемые заголовки и тело ответа (Слайд 45).  
Сервер это всё упакует в HTTP-ответ (объединив со своими заголовками) и вернёт клиенту.

Но этот вариант оказался не оптимальным ввиду увеличивающихся нагрузок.  
То есть создавать процесс на каждый запуск стало слишком накладно по времени (показать затраченное время на отработку запроса CGI).  
...
А веб-сервер и php-fpm будут обмениваться сообщениями по сети.
...
То есть сервер отправляет те же данные, что при использовании CGI, но в другом виде.  
...
Тут мы видим те же данные, типа `SERVER_PORT` и `SERVER_ADDR`, в `FCGI_PARAMS`, которые есть в массиве `$_SERVER` при работе CGI.
...
- `pool.d` — конфигурация пулов (набор запущенных интерпретаторов-«воркеров», работающих в этом процессе-демоне).  
  То есть в php-fpm работает пул интерпретаторов.  
  Также там есть мастер-процесс, который распределяет поступившие запросы между воркерами.  
...
То есть мы видим аналогичную ситуацию — он не знает, как запускать PHP-код.

По умолчанию Nginx имеет файл `/etc/nginx/mime.types`.  
Если его прочитать, то там будет набор сопоставлений расширение → content-type.  
Для файлов `.php` там сопоставлений нет.

Но давайте произведём базовую настройку сервера, чтобы он мог отправлять запросы php-fpm, если клиент обращается к PHP-файлам.

В дальнейшем я планирую сделать отдельное видео, посвящённое связке Nginx + php-fpm.  
Поэтому сейчас проведём лишь краткий обзор.

Все файлы конфигурации Nginx лежат в `/etc/nginx`.

Если вычленить основные точки конфигурации, требующие понимания, то это:

1. Файл `/etc/nginx/mime.types` — список расширений и соответствий им заголовка `content-type`.  
   Расширения для PHP-файлов там нет, и поэтому nginx отдаёт по умолчанию `content-type: application/octet-stream`, что приводит к скачиванию файла в браузере.

2. Конфигурации хостов расположены в директориях `sites-available` и `sites-enabled`.  
...
3. Конфигурирование запуска php-fpm производится в файле `/etc/nginx/sites-available/default`.  
...
- `fastcgi_pass unix:/run/php/php8.4-fpm.sock;` — адрес сокета, через который nginx будет осуществлять запросы к менеджеру процессов fpm.  
  То есть, если php-fpm и nginx работают на одной машине, то обмен сообщениями может идти через сокет. Но на продакшене это нередко работает именно по сети.
...
На основе всего вышесказанного я для себя построил такую карту этой технологии:  
Тут пронумированы основные направления и возможные подтемы.
