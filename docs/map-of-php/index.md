...
Ну, то есть когда дело дойдет до строки `cat -`, у пользователя появится возможность ввести текст с клавиатуры. 
...
То есть посмотреть на условный CLI профиль. 
...
То есть мы наглядно видим, что собой представляет CLI SAPI с точки зрения программиста на php.
...
В протоколе CGI просто парсится http запрос и стартовая строка и заголовки идут в переменные окружения, а тело запроса в stdin.  
...
Выполним запрос `http://localhost:8888/script.php`.

Резюме:

### Fast CGI

Сейчас, наш сервер запускает процесс php php-cgi который получает путь к вызываемому файлу через переменную окружения:

CGI:     Server -> php-cgi-runner -> php-cgi (то есть процес запустился, отработал и умер)

FastCGI: Server -> php-fpm (отдельный процесс демон ждущий подключения. Он работает постоянно. Получает данные по протоколу FastCGI)

При использовании FastCGI псевдо потоки типа php://stdin пустые, так как ОС не копирует родительские потоки. 
Она вообще не создает отдельный процесс на каждый запрос веб сервера. 
Для этого в FastCGI сделан новый псевдопотоки, типа php://input и тд.
Который FPM SAPI наполняет из сообщения веб-сервера по протоколу FastCGI.

По этому, если мы хотим унифицировать работу то во всех скриптах для веба следует использовать именно псевдопоток `php://input`.


Основной причиной появления FastCGI стало то что CGI создавал процесс php на каждый запрос веб сервера. 
По этому было решено создать демона, который будет запускаться один раз и обмениаться данными с веб сервером по сети или через сокет. 
То есть когда мы используем CGI то процесс php получал переменные окружения с заголовками http запроса и его тело, а тут решили передавать эти же данные но в особом бинарном формате. 
[FastCGI Specification](https://www.mit.edu/~yandros/doc/specs/fcgi-spec.html#SB)

То есть http сервер упаковыввает данные в соответствии с протоколом FastCGI. Он также парсит http запрос и стартовую строку с заголовками передает как FCGI_PARAMS
а тело запроса как FCGI_STDIN 
<p align="center">
  <img src="img/2.png" alt="спецификация FastCGI">
</p>
А FPM SAPI уже внутри себя разбирает данные и раскладывает их также в суперглобальные массивы и создает псевдо-потоки. 

Как результат, если мы установим веб сервер Nginx
и php-fpm то для программста на php останется привычный и знакомый  ему по CGI веб-профиль данных и псевдопотоков. 
