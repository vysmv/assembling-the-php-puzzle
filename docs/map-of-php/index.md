...
- CLI SAPI. Использутся для запуска интерпретатора в терминале. Терминал делает системный вызов ОС. и передаёт данные через STDIN (поток ввода), переменные окружения и параметры (аргументы) команды. CLI SAPI способно принять данные в таком формате и преобразовать их в удобоваримые для ядра. 
