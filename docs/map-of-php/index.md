...
И перейдем к практике в которой мы закрепим пройденный материал.

Для теста я подниму на VirtualBox пустой Debian 13 и буду проводить всю демонстрацию в терминале. 
Единственное это я добавлю - пробросы портов в настройках сети, чтобы с хостовой машины делать ssh и http запросы в гостевую ОС. 
...
sudo apt update
```
А потом установлю актуальную версию php:
...
Это CGI, FastCGI или запуск в терминале. Есть еще вариант где используется модуль для Apache, но я его затрону лишь вскользь, так как он не имеет важного значения с точки зрения фундаментального понимания (как нпрмер CGI) и при этом уже устаревший. 
...
Как я и говорил ранее, на практике используется именно FastCGI но для того чтобы его понять нужно сначала понять как работает CGI. 
...
В директории `/var/www/html/index.html` я создам пару новых файлов (style.css и script.php).
```css
html {
