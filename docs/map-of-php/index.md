...
То есть сделаем запрос к веб-серверу через http клиент, а веб-сервер вызовет php-cgi-runner как самостоятельный процесс. 
В скрипте php-cgi-runner указан шеба́нг #!/usr/bin/env php-cgi 
Так вот при запуске этого скрипта он считает переменные окружения и поймет какой файл мы пытаемся запустить. 
И самостоятельно запустит его. 
