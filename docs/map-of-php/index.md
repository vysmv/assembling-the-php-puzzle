...
(слайд 19) У интерпретатора есть 3 основных компонента:
```bash 
...
(слайд 25) **Наиболее часто встречающиеся среды исполнения PHP и их протоколы/механизмы обмена данными:**
...
> Важно!
>
> Обратите внимание, что каждое SAPI может опредлять разный набор доступных суперглобальных массивов. 
> Исходя из того, что передает среда. То есть например при запуске в терминале не будет заполнен массив $_POST и т.д.
> sapi с точки зрения php разработчика, это как раз различный набор данных предопределенных средой исполнения. 
> При запуске в терминале доступны одни массивы данных, а при запуске в вебе появятся свои специфические.  
> PHP-разработчик обращается к этому интерфейсу (в виде наборов массивов) и получает нужные ему данные.

> Важно!
> 
> Также важно осозновать что SAPI не только создает суперглобали для разработчика но и управляет жизненным циклом ядра. 
