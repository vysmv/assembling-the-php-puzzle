...
## Дисклеймер
Предполагается, что у вас уже есть опыт с php от года или вы опытный программист на других языках, который просто решил посмотреть на устройство этой технологии. Если вы вообще почти не имеете опыта, но в общем и целом любите разбираться в механизмах и вас заинтересовал php, то можете попробовать.

## Презентация
Привет всем заглянувшим. На повестке дня — построение карты мира PHP. Поехали!

## Введение
Около десяти лет назад я задался вопросом: а что такое PHP? То есть как реально устроен инструмент. В тот момент опыт в программировании у меня был очень поверхностным. Но для меня есть связь между комфортом использования инструмента и глубиной понимания его устройства.
...
```
<p align="center">
  <img src="img/1.gif" width="600" alt="компиляция">
</p>
...
#### 1. SAPI (Server API) 
<p align="center">
  <img src="img/3.gif" width="900" alt="Карта мира PHP">
</p>
Это интерфейс, через который ядро PHP (Zend Engine) взаимодействует с внешним миром.
Существуют разные среды запуска PHP-кода (например, CLI или FastCGI). Каждая среда по-своему обменивается данными с процессом PHP, используя собственный протокол.
...
*На практике: для запуска PHP-кода в консоли используют бинарь php-cli, а для работы через FastCGI — php-fpm и т. д.*

Как я уже упоминал выше, существует несколько вариантов SAPI для разных окружений, в которых может запускаться ядро. Каждая среда имеет свой протокол взаимодействия.
Например:
...
- внутренний API Apache,

- Существуют и другие, более экзотические варианты, типа `phpdbg` или `embed SAPI`. Но я с ними не сталкивался и пока что буду осознанно игнорировать.

Если кратко посмотреть на детали работы SAPI с конкретными протоколами, то выглядеть это будет так:

- CLI SAPI. Среда передаёт данные через STDIN (поток ввода), переменные окружения и параметры (аргументы) команды. CLI SAPI способно принять данные в таком формате и преобразовать их в удобоваримые для ядра. 
