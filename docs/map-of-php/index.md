...
Это CGI, FastCGI или запуск в терминале. Есть ещё вариант, где используется модуль для Apache (Apache + mod_php), но я его затрону лишь вскользь, так как он не имеет важного значения с точки зрения фундаментального понимания (как, например, CGI) и при этом редко используется в современном продакшене. Сегодня принят подход, при котором каждый инструмент решает свою узкую задачу. Подход, когда веб-сервер выполняет свою основную задачу, то есть отдачу статики, и при этом в том же процессе запускает интерпретатор, является устаревшим. Тем не менее, его можно встретить и по сей день на множестве проектов. Подход с разделением обязанностей даёт более высокий уровень надёжности и конфигурируемости системы, но и требует, как правило, большей квалификации, а следовательно, и стоимости обслуживания.
...
Как я и говорил ранее, в современной практике чаще всего используется FastCGI но для того чтобы его понять нужно сначала понять как работает CGI. 
...
Для удобства я разделю терминал на два окна и в каждом открою соответствующую директорию. 

В директории `/var/www/html/` я создам новый файл (script.php).
...
То есть веб-сервер умеет по умолчанию раздавать статические файлы но не умеет запускать скрипты написанные на том или ином ЯП. 
...
Если речь идет про Apache то мы можем либо:
- установить mod_php чтобы в запущенный процесс Apache был встроен интерпретатор php-кода.
- использовать интерпретатор php-cgi которому будем передавать путь к файлу который мы хотим запустить. 
...
А значит наш выбор это интерпретатор php-cgi. Который работает по протоколу CGI.
Сам протокол CGI был придуман, чтобы веб-сервер мог запускать программы написанные на разных языках программирования. 

То есть протокол CGI возник как ответ на вопрос - "как веб-сервер будет запускать программы". 

И постоили этот протокол на классическом принципе запуска программ в терминале. 

Давайте сделаем ответвление и разберем, то как запускаются программы в терминале, а потом снова вернемся к Apache2 и CGI.
...
Узнать реальное расположение бинарника можно при помощи `which ls`.
...
То есть, оболочка bash делает вызов fork(), раздваивается и в дочернем процессе вызывается execve,
что приводит к замене инструкций и данных дочернего процесса на указанную программу (в этом случае это ls).
Она получает переменные окружения, параметры вызова и потоки ввода\вывода от родительского процесса. 
...
if [ -t 0 ]; then
    echo "stdin подключён к терминалу. Возможен прямой ввод с клавиатуры."
    #cat -
else
    echo "Данные из stdin:"
    cat -
fi
echo
...
- `-t 0` — проверяет, подключён ли stdin к терминалу (TTY)  
  *То есть `-t 0` = проверить тип источника stdin:*  
  - TTY → **true**  
  - pipe / файл / сокет → **false**
...
То есть по смыслу, что передать содержание для stdin через pipe, файл или сокет условие [ -t 0 ] вернет false. И только прямой вызов в терминале ./bash_script вернет true.

- `cat -` — читает данные из stdin (многострочный ввод, pipe, файлы)
