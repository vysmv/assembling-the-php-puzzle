...
> **Интерпретаор** - это готовая к исполнению программа, сама по себе ничего полезного нам не дающая, но имеющая набор средств для интерпретации кода на PHP. Именно тут, в интерпретаторе, есть компилятор PHP-кода. В этом случае компилятор — часть интерпретатора).

Ему (интерпретатору) передается путь к файлу с исходным кодом на PHP. Который содержит бизнес логику. Пройдя ряд преобразований (в том числе и компиляцию), он исполнится в этом процессе интерпретатора.

Тут важно уловить, что ОС запускает именно процесс интерпретатора. То есть реально работает программа на Си которая принимает путь к исходному коду на PHP. Эта программа (интерпретатор) преобразовует этот код в некое подобие бинарника, а потом там же в памяти процесса исполняет его, при помощи виртуальной машины Zend Engine.  
