...
Мы можем открыть спецификацию и посмотреть как передаются данные [FastCGI Specification](https://www.mit.edu/~yandros/doc/specs/fcgi-spec.html#SB).

Далее, внутри php-fpm, SAPI парсит эти бинарные данные и наполняет суперглобальные массивы так как это было принято в CGI.
И создает специальные псевдопотоки типа `php://input` и `php://output`, чтобы ядро могло писать в них, а SAPI потом читало это и записывало в ответ в соответствии с протоколом FastCGI.
