...
И теперь, если мы перейдем по адресу `http://localhost:8888/`, то увидим приветственную страницу веб-сервера Apache2.
...
А потом попробовать сделать запрос на `http://localhost:8888/script.php`, 
...
В php скриптах могут быть проблемы при попытке чтения или записи в файлы /dev/stdin и /dev/stdout, но там есть удебные псевдопотоки типа php://stdin и php://stdout. 

### CLI профиль

Но давайте снова вернемся к нашему скрипту на `PHP`.
...
Также там есть мастер процесс распределяет равномерно поступившие запросы между воркерами. 
...
Далее мы реализуем базовую конфигурацию сервера, чтобы он начал отправлять запросы к php-fpm в случае запросов к php-файлам.

В дальнейшем я планирую сделат отдельное видео посвященное связке Nginx + php-fpm.

Так вот коратки обзор конфигурации будет выглядеть так:
...
- конфигурации хостов расположены в директориях sites-available (физические конфиги хостов), sites-enabled (симлинки включенных хостов на директорию sites-available. Добавление ссылки сюда говорит о том, что веб-сервер должен применить этот конфиг).

- конфигурирование php-fpm производится в `/etc/nginx/sites-available/default` таким фрагментом кода
```bash
location ~ \.php$ {
...
- разделили понятие PHP на язык и интерпретаор
- разобрали три основных компонента интерпретатора
- поняли основополагающий принцип запуска программ в терминале и его производную  CGI
- в кратце познакомились с FastCGI
